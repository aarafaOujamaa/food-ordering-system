...
public class OrderDomainServiceImpl implements OrderDomainService {
    ...
    @Override
    public OrderCancelledEvent cancelOrderPayement(Order order, List<String> failureMessages) {
    ...
    private void setOrderProductInformation(Order order, Restaurant restaurant) {
        order.getOrderItems().forEach(orderItem -> restaurant.getProducts().forEach(restaurantProduct ->
        {
            ...
            if (currentProduct.equals(restaurantProduct)) {
                currentProduct.updateWithConfirmedPrice(restaurantProduct.getName(),
